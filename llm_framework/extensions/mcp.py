...
class MCPBridge:
    ...
    def _make_proxy(self, mcp_tool: dict):
        ...
        async def proxy_fn(**kwargs):
            ...
            if transport == "stdio":
                coro = bridge._stdio_call("tools/call", args_payload)
                result = (
                ...
                result = await bridge._http_call("tools/call", args_payload)
            parts = [
                c["text"]
...
class MCPManager:
    "Aggregates multiple MCPBridge connections and exposes a unified tool list."

    def __init__(self, bridges: list):
        """
        Args:
            bridges: List of `MCPBridge` instances to aggregate. All are entered as a single context manager.
        """
        self.bridges = bridges
        self._exit_stack = contextlib.AsyncExitStack()

    async def __aenter__(self):
        # single stack tears down all bridges together on exit
        for bridge in self.bridges:
            await self._exit_stack.enter_async_context(bridge)
        return self
    ...
    async def get_all_tools(self):
        ...
        tasks = [bridge.get_tools() for bridge in self.bridges]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        tools = []
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                log.warning("MCP bridge %d failed to load tools: %s", i, result)
            else:
                tools.extend(result)
        return tools
